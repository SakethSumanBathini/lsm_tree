...
// Arena: simple bump-pointer allocator for Lock-Free SkipList nodes.
// Memory is reclaimed entirely when the Arena (and Memtable) is destroyed.
// Thread-safe: uses atomic CAS for fast-path bump allocation and a mutex
// to protect new block allocation.
class Arena {
public:
